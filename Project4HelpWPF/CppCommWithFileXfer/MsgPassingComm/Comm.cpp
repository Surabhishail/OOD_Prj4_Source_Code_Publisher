...
const std::string saveFilePath = "../SaveFiles";//AM SAVE FILE NAME Send file path and rcv file path
const std::string sendFilePath = "../SendFiles";//const so not changeable
...
bool Sender::sendFile(Message msg)
{
  if (!msg.containsKey("file"))
    return false;
  std::string fileSpec = sendFilePath + "/" + msg.file();//AM 
...
void Comm::start()//AM(const std::string& rcvPath,const std::string& sendPath)
{
